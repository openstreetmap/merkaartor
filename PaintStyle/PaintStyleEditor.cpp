...
void PaintStyleEditor::on_RemoveButton_clicked()
{
	...
	if (idx >= thePainters.size()) return;
...
void PaintStyleEditor::on_Key_textEdited(const QString&)
{
	unsigned int idx = static_cast<unsigned int>(PaintList->currentRow());
	if (idx >= thePainters.size()) return;
	FeaturePainter& FP(thePainters[idx]);
...
void PaintStyleEditor::on_Value1_textEdited(const QString&)
{
	on_Key_textEdited("");
}

void PaintStyleEditor::on_Value2_textEdited(const QString&)
{
	on_Key_textEdited("");
}
