...
#ifndef GGL_ALGORITHMS_INTERSECTION_SEGMENT_HPP
#define GGL_ALGORITHMS_INTERSECTION_SEGMENT_HPP
...
enum intersection_type
{
    ...
    is_collinear_no,
    is_collinear_one,
    is_collinear_connect,
    is_collinear_overlap,
    is_collinear_overlap_opposite,
    is_collinear_connect_opposite
};
...
struct intersection_result
{
    ...
    inline connection_type get_connection_type() const
    {
        ...
        {
            if (math::equals(rb, 0.0)) return is_connect_s2p1;
            if (math::equals(rb, 1.0)) return is_connect_s2p2;
            if (math::equals(ra, 0.0)) return is_connect_s1p1;
            if (math::equals(ra, 1.0)) return is_connect_s1p2;
        }
...
/*!
    ...
    \until }
*/
template <typename P, typename S1, typename S2, typename OutputIterator>
inline intersection_result intersection_segment(const S1& s1, const S2& s2, OutputIterator out)
{
    ...
    coordinate_type const dx1 = get<1, 0>(s1) - get<0, 0>(s1);
    coordinate_type const dx2 = get<1, 0>(s2) - get<0, 0>(s2);
    coordinate_type const dy1 = get<1, 1>(s1) - get<0, 1>(s1);
    coordinate_type const dy2 = get<1, 1>(s2) - get<0, 1>(s2);

    coordinate_type const wx = get<0, 0>(s1) - get<0, 0>(s2);
    coordinate_type const wy = get<0, 1>(s1) - get<0, 1>(s2);

    // Calculate determinants - Cramers rule
    coordinate_type const d = (dy2 * dx1) - (dx2 * dy1);
    coordinate_type const a = (dx2 * wy) - (dy2 * wx);
    coordinate_type const b = (dx1 * wy) - (dy1 * wx);

    if (math::equals(d, 0))
    {
        if (math::equals(a, 0) && math::equals(b, 0))
        {
            bool vertical = math::equals(dx2, 0);
            ...
            else
            {
                ...
                if (math::equals(dx1, 0) && math::equals(dy1, 0))
                {
                ...
                if (math::equals(dx2, 0) && math::equals(dy2, 0))
                {
                    P p(s2.first);
                    *out = p;
                    ++out;
                ...
                result.ra = vertical ? (wy / dy2) : (wx / dx2);
                result.rb = vertical
                ...
                *out = P(get<0, 0>(s2) + result.ra * dx2, get<0, 1>(s2) + result.ra * dy2);
                ...
                if (math::equals(result.ra, result.rb))
                {
...
#endif // GGL_ALGORITHMS_INTERSECTION_SEGMENT_HPP
