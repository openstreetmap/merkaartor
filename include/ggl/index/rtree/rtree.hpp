...
#ifndef GGL_INDEX_RTREE_RTREE_HPP
#define GGL_INDEX_RTREE_RTREE_HPP
...
public:
    ...
    /**
     * \brief Remove the element with 'key'
     */
    inline void remove(Point const& key)
    {
        /// it's the same than removing a box of only one point
        this->remove(Box(key, key));
    }
     ...
     */
    void remove(Box const& box, const Value& value)
    {
        try
        {
            node_pointer leaf;
    ...
    /**
     * \brief Inserts an element with key and value.
     */
    inline void insert(Point const& key, Value const& value)
    {
        // it's the same that inserting a box of only one point
        this->insert(Box(key, key), value);
    }
...
#endif // GGL_INDEX_RTREE_RTREE_HPP
