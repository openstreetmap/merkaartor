...
#include <ggl/util/graticule.hpp>
...
/*!
    ...
    \tparam C coordinate system, optional, should includes degree/radian indication, defaults to geographic<degree>
...
public:
    ...
    inline point_ll(const longitude<T>& lo, const latitude<T>& la) : point<T, D, C>(lo, la) {}

    /// Constructor with latitude/longitude
    inline point_ll(const latitude<T>& la, const longitude<T>& lo) : point<T, D, C>(lo, la) {}
...
{
    template <size_t I>
    static inline T get(const point_ll<T, C, D>& p)
    {
        return p.template get<I>();
    }

    template <size_t I>
    static inline void set(point_ll<T, C, D>& p, const T& value)
    {
