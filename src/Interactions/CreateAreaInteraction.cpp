...
void CreateAreaInteraction::finishRoad(CommandList* L)
{
    ...
    {
        theRelation = g_backend.allocRelation(CUR_DOCUMENT->getDirtyOrOriginLayer());
        L->add(new AddFeatureCommand(CUR_DOCUMENT->getDirtyOrOriginLayer(),theRelation,true));
        if (M_PREFS->getAutoSourceTag()) {
            QStringList sl = CUR_DOCUMENT->getCurrentSourceTags();
            if (sl.size())
...
void CreateAreaInteraction::addToRoad(QMouseEvent* anEvent, Feature* Snap, CommandList* L)
{
    ...
    if (!To)
    {
        To = g_backend.allocNode(CUR_DOCUMENT->getDirtyOrOriginLayer(), XY_TO_COORD(anEvent->pos()));
...
void CreateAreaInteraction::snapMouseReleaseEvent(QMouseEvent* anEvent, Feature* aFeature)
{
    ...
    if ( !panning() ) {
        ...
        else
        {
            ...
            view()->invalidate(true, true, false);
            if (theRelation)
                PROPERTIES_DOCK->setSelection(theRelation);
            else
        ...
        if (EndNow) {
            if (theRelation)
                ...
                PROPERTIES_DOCK->setSelection(LastRoad);
            LastRoad = NULL;
            theRelation = NULL;
            HaveFirst = false;
            EndNow = false;
